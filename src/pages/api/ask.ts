...
// valores de api de openai
const openai = new OpenAI({
    apiKey: import.meta.env.OPENAI_KEY
})

// ruta para realizar una pregunta a un banco de conocimientos
export const GET: APIRoute = async ({ request }) => {
    const url = new URL(request.url)
    const id = url.searchParams.get('id')
    const question = url.searchParams.get('question')

    // validar que exista documento a revisar
    if (!id) {
        return new Response('Missing id', { status: 400 })
    }
    // validar que exista una pregunta a realizar
    if (!question) {
        return new Response('Missing question', { status: 400 })
    }

    // leer el documento y realizar la pregunta con el banco de conocimientos
    const txt = await readFile(`public/text/${id}.txt`, 'utf-8')

    // enviar respuesta a la pregunta con el banco de conocimientos
